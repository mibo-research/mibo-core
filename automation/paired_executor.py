...
from raw_archive import archive_failure, archive_success, write_deviation
...
def preflight(*, manifest_path: Path, freeze_path: Path, authorization_path: Path, data_root: Path, now: datetime | None = None) -> tuple[list[dict], dict, datetime, datetime]:
    ...
    if any(r.get("configuration_freeze_sha256") != freeze_sha for r in rows):
        raise ValueError("manifest is not bound to the supplied provider-freeze file")
...
def execute(*, manifest_path: Path, freeze_path: Path, authorization_path: Path, data_root: Path, timeout_s: int = 180) -> dict[str, int]:
    ...
    rows, _auth, _field_start, field_close = preflight(
        manifest_path=manifest_path, freeze_path=freeze_path, authorization_path=authorization_path, data_root=data_root
    )
    profiles = _freeze_profiles(freeze_path)
    prompts = _prompt_map()
    summary = {"valid": 0, "failed_attempts": 0, "retries_scheduled": 0, "timing_deviations": 0}
    ...
    for (lineage, item_id), block in grouped.items():
        ...
        while queue:
            ...
            if not isinstance(profile, dict):
                raise ValueError(f"{lineage} is missing request_profile in provider freeze")
            try:
                ...
                if decision.retry:
                    queue.append((parse_utc(decision.due_at_utc), _clone_retry_row(row, int(decision.next_attempt))))
